...
class SimpleNetwork:
    """A simple feedforward network with sigmoid activation functions."""

    @classmethod
    def random(cls, *layer_units: int):
        """Initializes a neural network with random weights.

        :param layer_units: number of neurons in each layer
        """
    ...
    def __init__(self, *layer_weights: np.ndarray):
        """Initializes the network with given weight matrices.

        :param layer_weights: Weight matrices defining the network layers
        """
    ...
    def predict(self, input_matrix: np.ndarray) -> np.ndarray:
        """Predict outputs by forward propagation.

        :param input_matrix: input data
        :return: output predictions (continuous values between 0 and 1)
        """
    ...
    def predict_zero_one(self, input_matrix: np.ndarray) -> np.ndarray:
        ...
        :param input_matrix: input data
        :return: binary output predictions (0 or 1)
        """
    ...
    def gradients(self,
        ...
        :param input_matrix: input data
        :param output_matrix: true labels
        :return: gradients for each weight matrix
        """
        ...
        for i in reversed(range(len(self.weights))):
            delta = error * self.sigmoid_gradient(zs[i])
            grad = np.dot(activations[i].T, delta) / input_matrix.shape[0]
    ...
    def train(self,
        ...
        for _ in range(iterations):
            ...
            self.weights = [w - learning_rate * grad
                            for w, grad in zip(self.weights, grads)]
